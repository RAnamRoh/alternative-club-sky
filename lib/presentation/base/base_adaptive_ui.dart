...
abstract class BaseAdaptiveUiState<
  ...
  void _baseStateListener() {
    ...
    if (baseState is ShowSessionErrorDialogueState) {
      _showSessionErrorDialogue(
        baseState.iconData,
        baseState.title,
        baseState.buttonName,
        baseState.onPressed,
      );
    }
  }
  ...
  void _showSessionErrorDialogue(
    IconData iconData,
    String title,
    String buttonName,
    ...
    showDialog(
      context: context,
      builder: (context) => SessionErrorDialogue(
        iconData: iconData,
        title: title,
  ...
  void _handleError({
    ...
    if (error is BasicException) {
      msg = error.message;
    }
    if (error is SessionException) {
      _showSessionErrorDialogue(Icons.error, 'Session Expired', 'Re-login', () {
        _navigate(
          context: context,
